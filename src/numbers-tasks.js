...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalarProductXY = x1 * x2 + y1 * y2;
  const moduleX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const moduleY = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(scalarProductXY / (moduleX * moduleY));
}
...
function toNumber(value, def) {
  return String(value) === String(Number(value)) ? Number(value) : def;
}
...
function getSumOfNumbers(x1, x2, x3) {
  let sum = 0;
  let maxLengthAfterDot = 1;
  const arr = [x1, x2, x3];
  arr.forEach((x) => {
    const parts = x.toString().split('.');
    if (parts.length === 2) {
      maxLengthAfterDot =
        parts[1].length > maxLengthAfterDot
          ? parts[1].length
          : maxLengthAfterDot;
    }
  });

  maxLengthAfterDot = 10 ** maxLengthAfterDot;

  arr.forEach((x) => {
    sum += x * maxLengthAfterDot;
  });

  return sum / maxLengthAfterDot;
}
